...
int crearEntrenadores(char** objetivos, t_log* logger) {
	...
	entrenador->objetivo->sgte = NULL;
	entrenador->sgte = NULL;
	while (objetivos[i] != NULL) {
		entrenador->objetivo->nombre = objetivos[i];
		log_info(logger, "iterate nro %i: %s", i, entrenador->objetivo);
		i++;
	}
	char* token = strtok(objetivos[i], "|");
	while (token != NULL) {
		char* direccion;
		if (direccion = buscarEn(entrenador, token)) {
			actualizarEn(entrenador, &direccion);
		} else {
			encolar(entrenador, token);
		}
		token = strtok(NULL, "|");
	}
	return i;
